...
export default function SpinningBoxWithCanvas() {
    ...
    const [rectAngle, setRectAngle] = useState(135);
    const [rectPosition, setRectPosition] = useState(0.5);
    ...
    const updateDrawing = (newDrawing, arm, i) => {
        ...
        if (arm.drawingEnabled) {
            ...
            const tipX = pivotX + Math.cos(arm.angle * Math.PI / 180) * (-arm.position * 10);
            const tipY = pivotY + Math.sin(arm.angle * Math.PI / 180) * (-arm.position * 10);
            ...
            const platformX = tipX * Math.cos(platformAngle * Math.PI / 180) - tipY * Math.sin(platformAngle * Math.PI / 180);
            const platformY = tipX * Math.sin(platformAngle * Math.PI / 180) + tipY * Math.cos(platformAngle * Math.PI / 180);
            ...
            const relativeX = (platformPositionX - robot.platform.width / 2) * Math.cos(-platformRotation) - (platformPositionY - robot.platform.height / 2) * Math.sin(-platformRotation) + robot.platform.width / 2;
            const relativeY = (platformPositionX - robot.platform.width / 2) * Math.sin(-platformRotation) + (platformPositionY - robot.platform.height / 2) * Math.cos(-platformRotation) + robot.platform.height / 2;
    ...
    useEffect(() => {
        armIntervalRef1.current = setInterval(() => {
            setDrawing((prevDrawing) => {
                let newDrawing = [...prevDrawing];

                return updateDrawing(newDrawing, robot.arm[0], 0);
            });
        }, 100);
    ...
    useEffect(() => {
        armIntervalRef2.current = setInterval(() => {
            setDrawing((prevDrawing) => {
                ...
                return updateDrawing(newDrawing, robot.arm[1], 1);
            });
        }, 100);
    ...
    useEffect(() => {
        armIntervalRef3.current = setInterval(() => {
            setDrawing((prevDrawing) => {
                ...
                return updateDrawing(newDrawing, robot.arm[2], 2);
            });
        }, 100);
    ...
    useEffect(() => {
        armIntervalRef4.current = setInterval(() => {
            setDrawing((prevDrawing) => {
                ...
                return updateDrawing(newDrawing, robot.arm[3], 3);
            });
        }, 100);
                ...
                >
                    ...
                    >
                        ...
                        {drawing.map(([x, y], i) => {
                            ...
                            return <circle key={i} cx={x * 10} cy={y * 10} r="1" fill="black" />
                        })}
                    </motion.g>
                    <rect
                        id="frame"
                    ...
                    <div>
                        ...
                        <label>
                            Size: <input
                                ...
                                onChange={(e) => setRobot({ ...robot, frame: { ...robot.frame, width: Number(e.target.value), height: Number(e.target.value) } })}
                            />in
                        </label>
                        <label>
                            Thickness: <input
                    ...
                    <div>
                        ...
                        <label>
                            Length: <input
                                ...
                                onChange={(e) => {
                                    setRobot({ ...robot, arm: robot.arm.map((arm) => ({ ...arm, length: Number(e.target.value), maxPosition: Number(e.target.value) - arm.minPosition, position: Number(e.target.value) - arm.minPosition })) })
                                }}
                            />in
                        </label>
                        <label>
